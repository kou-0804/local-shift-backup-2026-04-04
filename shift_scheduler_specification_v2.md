...
**データサンプル**:
```csv
技師ID,病院MR,CLMR,CT,病CT,ア,心,ク,ポ,精,MG,DR,HB,OP,PICC,入,出,超遅
T001,-,-,B,C,-,-,-,-,-,-,-,-,-,-,-,-,-
T002,B,B,A,-,-,-,B,-,A,-,A,-,-,-,-,-,-
T009,-,-,B,A,A,A,B,-,-,-,-,B,-,-,-,-,-
...
...
| CLMR | クリニッCLMRI | MRI | 5 | 5 | 5 | 5 | 5 | 3 | 0 | なし |
...
def derive_night_skills(staff_id: str, day_skills: dict) -> dict:
    ...
    # 夜勤MR: 病院MR または CLMR が B以上
    if day_skills.get('病院MR') in ['A', 'B'] or day_skills.get('CLMR') in ['A', 'B']:
...
| NH-06 | モダリティ必須スキル | 3名の組み合わせで最低1名はMR、最低1名はアンギオ、最低1名は心臓カテのスキルを持つこと。(ハード制約) | 解なし |
...
| DH-09 | 6連勤禁止 | **絶対に6連勤を超えない（7日目は必ず「休」となる）**<br>日勤だけでなく「夜勤(夜希含む)」「明け(○)」「指定業務(講,会議,業配など)」すべてを勤務日（稼働日）として連続カウントし、向こう数日間の確定済み勤務状況（先読み機能）を考慮して7連勤が不可避になる前に強制休を挿入する。<br>※ただし、配置予定日に明示的な予定申請（会議や研究など）がある場合は強制休による上書きを避け、申請を優先する（休夜などの不自然な状態を回避）。 | 解なし |
...
| DS-04 | 勤務場所の均等化 | 全ての勤務場所において、各技師の配置回数が均等になるよう累積回数に応じたペナルティを課す。（「特定の人のみが専門業務を独占する」事態を防止）<br>※**「M遅」**については、配置の偏りを極力なくすため**非常に強力なペナルティ（回数×10,000）**を適用する。 | 20倍積 (M遅は10,000倍積) |

### 5.3 個人特例ルール（ク配置指定）

特定の技師に対する、月間指定回数および特例特権を処理する。

| 対象者 | 勤務場所 | ルール詳細 |
|--------|----------|------------|
| 小野 雄一朗、川名 佑樹 | ク（クリニック一般） | ・月間にそれぞれ最大6回まで「ク」に優先配置するようシステムに強力なインセンティブ（+500,000）を付与。<br>・6回に達した時点でその月は「ク」への配置をハード制約で禁止。<br>・**重複禁止**: お二人が同日に「ク」に配置されることをハード制約で全面禁止。<br>※注意: 特化スキル（HBや心など）の必要人数や、女性必須枠（PB-06）等の絶対的ハード制約が優先されるため、スケジュールによっては6回に未達（4〜5回）となる場合がある。 |

### 5.4 個人特例ルール（PET配置指定）
| 対象者 | 勤務場所 | ルール詳細 |
|--------|----------|------------|
| 石川 和弥 | PET | ・対象月の**「第4火曜日」**に限定して、強制的に勤務場所を「PET」として割り当てる。<br>・対象日がすでに夜勤明けなどである場合や、スタッフ自身による休暇希望（☆、休）が提出されている場合は、その休暇希望の方が優先される。<br>・ソルバーの最適化ループを完全にバイパスしてExcelに出力されるため、マスター設定への影響はない。 |

### 5.5 特例ルール（MRI専任スタッフの優先配置）
| 対象者 | 勤務場所 | ルール詳細 |
|--------|----------|------------|
| 動的判定 | 病MR, CLMR, M遅 | ・マスター読み込み時に**「MRI系業務のスキルのみを有し、他のモダリティスキルを持たないスタッフ」**を動的に特定する。<br>・対象スタッフがMRI関連枠（病MR, CLMR, M遅）に配置される際、通常の50倍にあたる超特大ボーナス（+500,000）を付与し、複数スキル担当者よりも優先してMRI枠を確保させる。 |

### 5.6 休日（9日休）自動配置アルゴリズムと代休処理

日勤のスケジューリング完了後、以下の手順でシステムが自動的に「休」を配置し、各技師の月間休日数が**必ず9日**になるよう調整を行う。

#### 5.6.1 休日の定義
システムのカウントにおいて「休日」と見なされるのは以下の条件を満たす日である。
* 明示的な希望休（`★`, `☆`, `◆`, `★連` など）
* 特別な休暇・公休（`退職`, `出/☆`, `☆/(発)` など）
* 配置のない日曜日・国民の祝日
* 自動配置された「休」

**※注意**: 「夜勤明け（`○`）」、「午後休（`17休`）」、「業務関連（`研(聴)`, `業出` など）」は**稼働日（勤務）**として扱い、休日のカウントには含めない。

#### 5.6.2 自動休（休）の配置アルゴリズム
不足している休日数（9日 - 上記の休日カウント）を割り出すと、どこにも業務がアサインされていない「空欄（フリー）」の平日に「休」のマークを自動配置する。
配置する際は、**連休（休みの連続）を避ける**ため、以下の動的スコアリング（分散アルゴリズム）を用いる。
1. 現在の「空欄」候補日のうち、**すでに確定している他の休み（日祝や希望休、配置済みの「休」）から最も日数が離れている日**を優先的に選択する。
2. 配置後、その日は「休み」として確定し、直ちに他の候補日の距離スコアを再計算する。
3. いかなる配置においても、前後数日間をチェックし「7連勤以上」が発生する割り当ては即座にキャンセルされ、別の日を再探索する。

#### 5.6.3 パワーバランス・人員確保による代休（代休）処理
特定の技師（MRI専任者など）において、必須業務へのアサインが多すぎる結果「空欄（フリー）」の日が不足し、自動休を限界まで埋めても9日に到達しないケースが存在する。
この場合、システムは「業務を『休』で強制上書きして現場を破綻させること」を避け、現在のスケジュールを維持したまま、不足分を**代休（最大1日）**として付与する。
* **出力**: 勤務表Excelの最右翼（統計列の最後）に「代休」列が自動生成され、付与対象者には「1」が記載される。
* 1ヶ月あたり1人につき付与される代休は上限1日までとする。

### 5.7 拘束（オンコール）自動配置処理

日勤および休日のスケジューリング完了後、システムの最終調整段階として「拘束（オンコール）」の自動配置を行う。1日につき2名（第1拘束、第2拘束）が割り当てられる。

#### 5.7.1 拘束の基本ルール
拘束業務は、以下の「平日」「土曜・祝前日」「日曜・祝日」の3パターンの勤務条件に応じてそれぞれ適任者を抽出する。
※大前提として、以下に該当するスタッフはその日のオンコール候補から完全に除外される。
1. 技師マスタの「拘束可否」が『×』のスタッフ
2. 当日の予定申請が「17業」および「17休」となっているスタッフ（17時以降の予定・休みが確約されているため）

| パターン | 人数 | 時間 | 対象者の条件 |
|----------|------|------|-------------|
| **平日** | 2名 | 17:00〜翌08:00 | **両者共通:** 当日に通常日勤を行っており、かつ翌日も通常日勤の予定者（夜勤、遅番系、および休みの者はすべて除外）であること。<br>※自宅待機オンコールの基本ルールとして、翌日も通常日勤である者が前提となる。 |
| **土・祝前日** | 2名 | 17:00〜翌08:00 | **第1拘束:** 翌日（日・祝）の夜勤（当直）予定者<br>**第2拘束:** 翌日（日・祝）の日勤予定者<br>※当日は出勤の有無を問わず、上記に該当する者が自宅待機オンコールに入る。|
| **日・祝日** | 2名 | 第1: 08:00〜17:00<br>第2: 17:00〜翌08:00 | **第1拘束(日中):** その日の明け（○）となっているスタッフ<br>**第2拘束(夜間):** その日（日・祝）に出勤して日勤業務に入っているスタッフの誰か |

#### 5.7.2 割り当ての均等化と経験年数・スキル補正
1. **回数の均等化:** 特定のスタッフに負担が偏らないよう、配置の際は常に**「当月内のこれまでの拘束回数」が最も少ない人間を優先**して割り当てる動的ソートアルゴリズムを用いる。
2. **経験年数によるペアリング補正:** 拘束ペアが若手（経験年数3年未満）同士になることを防ぐため、第1拘束者が経験年数3年未満だった場合、第2拘束者の選定において他の若手（3年未満）に対してペナルティスコア（+100回相当）を付与し、可能な限り中堅・ベテランがペアに選ばれるようシフトする。
3. **夜勤HBフェールセーフ補正:** 同日夜の「夜勤」担当者3名の中に誰も「HB」スキル（ランクB以上）を持つ者がいなかった場合、拘束候補者の中で「HB」スキルを持つ者に対して強力な優先スコア（-1000回相当）を与え、確実に対象日の拘束アサインに組み込まれるように調整する。

#### 5.7.3 出力仕様
* Excelファイルの最下部、各個人行が終わった後に「第1拘束」「第2拘束」専門の新しい行が2行追加され、対象日ごとに配置されたスタッフの「苗字（カッコなどを除いた短縮名）」が自動で書き出される。
...
#### 7.1.5 CLMR（クリニッCLMRI）

マスターの基本パワーバランス設定をバイパスし、プログラム内のハードコードで以下の厳密な曜日別条件を適用する。

| 曜日 | CLMR枠数 | ランク要件（ハード制約） | 育成・その他の要件（ソフト制約など） |
|------|------|------------|------|
| 月火木土 | 4枠 | Aランク2名以上, A+Bランク合計3名以上 | Dランクは上限1名。<br>※さらに「可能な限りDランクを1名配置して経験を積ませる」よう+500のソフト制約ボーナスを付与 |
| 水曜 | 4枠 | **Aランク3名以上, A+Bランク合計4名以上** | **C・Dランク完全禁止** |
| 金曜 | 5枠 | Aランク2名以上, A+Bランク合計4名以上 | M遅枠がないため5枠。<br>※Dランクは上限1名とし、Dランク1名配置推奨ソフトボーナス(+500)を付与 |
...
2025-12-01,月,CLMR,5,T003,矢野　昌男,T011,渡邉　一博,...
...
```
...
| 2.3 | 2026-02-12 | 「M遅」シフトの追加（それに伴うCLMRの月〜木配置数減）、日祝と明休日祝の夜勤回数均等化（ハード制約＋増分ペナルティ）の実装を反映 |
