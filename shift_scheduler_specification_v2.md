...
├── 業務拡大マスタ.csv          # 育成ペアリング設定 [NEW]
├── 公休数マスタ.csv            # 月別の公休日数設定 (目標: 8〜10日)
├── 予定申請.csv                  # 月別の予定申請（全スタッフ統合版）
└── 夜勤回数_確定版.csv         # 月別の夜勤回数指定マス [UPDATE]
```
...
| CLMR | クリニックMRI | MRI | 5 | 5 | 5 | 5 | 5 | 3 | 0 | なし |
...
| 入 | 病院一般 | 一般 | 1 | 1 | 1 | 1 | 1 | 1 | 0 | なし |
| 病L | 病院リーダー | 一般 | 1 | 1 | 1 | 1 | 1 | 1 | 0 | なし |
...
**制約**: 全技師の回数合計 = 対象月の日勤対象日数 × 3
**管理**: `夜勤回数_確定版.csv` をマスターデータとして扱い、ソルバーはこの回数を最優先で遵守する。
...
def derive_night_skills(staff_id: str, day_skills: dict) -> dict:
    ...
    # 夜勤MR: 病院MR または CLMR が C以上 [UPDATE: ランク緩和]
    if day_skills.get('病院MR') in ['A', 'B', 'C'] or day_skills.get('CLMR') in ['A', 'B', 'C']:
        night_skills['night_mr'] = True
    
    # 夜勤アンギオ: ア が C以上 [UPDATE: ランク緩和]
    if day_skills.get('ア') in ['A', 'B', 'C']:
        night_skills['night_angio'] = True
    
    # 夜勤心カテ: 心 が C以上 [UPDATE: ランク緩和]
    if day_skills.get('心') in ['A', 'B', 'C']:
        night_skills['night_cath'] = True
        
    # 夜勤HB: 病院MR が B以上 [NEW]
    if day_skills.get('病院MR') in ['A', 'B']:
        night_skills['night_hb'] = True
...
| DS-04 | 勤務場所の均等化 | 全ての勤務場所において、各技師の配置回数が均等になるよう累積回数に応じたペナルティを課す。（「特定の人のみが専門業務を独占する」事態を防止）<br>・**カウントの正規化**: `ク遅` や `ク/17業` などの派生シフトは、ベースとなる `ク` の回数として合算集計される。<br>・**M遅の扱い**: `M遅` は `CLMR` の回数として集計される。<br>・**重みの強化**: 特定業務への過集中を防ぐため、ペナルティの重みを大幅に引き上げ（20 → 150）。 | 150倍積 (M遅は10,000倍積) |
...
### 5.6 休日（月別公休）自動配置アルゴリズムと代休処理

日勤のスケジューリング完了後、以下の手順でシステムが自動的に「休」を配置し、各技師の月間休日数が**指定された目標日数**になるよう調整を行う。

#### 5.6.1 休日の目標設定
以前は一律「9日」固定であったが、現在は `公休数マスタ` に基づき、月ごとに異なる目標日数（8日〜10日）を設定可能。
* **マスタ設定例**: 2026年4月=9日、5月=10日、2027年2月=8日。
* **フォールバック**: マスタに記載がない月はデフォルトの「9日」を適用する。
* 明示的な希望休（`★`, `☆`, `◆`, `★連` など）
* 特別な休暇・公休（`退職`, `出/☆`, `☆/(発)` など）
* 配置のない日曜日・国民の祝日
* 自動配置された「休」
...
> **【2026-07 更新: 余剰の空欄化】** 以前は空欄平日を**全件**「休」に変換していたため、在籍が需要を上回る月は公休が規定（例: 9日）を大きく超過していた（例: 7月は在籍の約半数が9日超）。
> 現在は**規定到達分（`blanks_quota = max(0, 目標 - 明示公休)`）だけ**を月内に分散して「休」化し、**余剰の空欄はそのまま空欄**（未割当・公休カウント外）として残す。
> ⇒ 公休カウント = 明示公休 + 付与した「休」 + 0.5×半休 で、**規定で頭打ち**（明示休（★☆/育休等）が規定を超える場合はその実数）。`assign_monthly_off_days`（work割当を変えない最終段）と `stats_engine.recompute_off_daikyu`（Web再計算）で同一ロジックにより整合。「休」でも空欄でも非勤務のため6連勤判定には等価。
...
#### 5.6.4 公休属性の可視化と区別（Excel表示）
スタッフの休みには「本人の希望」と「システムによる自動調整」の2種類があり、Excel出力時に背景色で区別される。
* **本人希望休 (`★`, `☆`, `◆`, `★連` など)**:
    * 予定申請に基づき配置。
    * 表示: **赤背景** (`FFCDD2`)。
* **システム自動公休 (`休`)**:
    * 公休目標達成や連勤防止のためにPhase 1またはポスト処理で自動配置。
    * 表示: **グレー背景** (`D3D3D3`)。

### 5.8 業務拡大（育成ペアリング）機能

特定のモダリティにおいて、ベテラン技師（指導者）と若手（育成対象者）をペアで配置し、実戦形式でのトレーニングを促進する機能。

#### 5.8.1 指導ルール設定
**ファイル名**: `業務拡大マスタ_確定版.csv`
* **対象モダリティ**: `ア`, `心`, `HB`, `病CT` など
* **指導者名**: ランクA以上のベテランをカンマ区切りで指定。
* **育成対象者名**: トレーニングを受ける若手をカンマ区切りで指定。

#### 5.8.2 アルゴリズムの挙動
1. **ダミー枠の生成**: 指導中であることを示すため、対象日に `(ア)` や `(心)` といったカッコ付きのダミー勤務場所を内部的に生成する。
2. **ペアリング制約**:
   - 育成対象者が選ばれるためには、その日に必ず「指定された指導者のうち少なくとも1名」がベースとなる業務（例：`ア`）に配置されている必要がある。
   - 指導者が配置されていない日は、育成対象者はそのモダリティのダミー枠には配置されない。
3. **カウントの扱い**: 均等化のカウントにおいて、ダミー枠 `(ア)` への配置はベース業務 `ア` の実績として正規化して加算される。
4. **優先順位（一人集中育成型）**: `業務拡大マスタ` の育成対象者リストにおいて、**左側に記載されているスタッフを最優先**して配置する。そのスタッフが目標のスキルレベルに達し、マスタから削除されるまで集中的にトレーニング機会が与えられる。
5. **スキルチェックのバイパス**: 育成用ダミー枠（カッコ付きの場所）に配置される際は、標準のスキルマスタによる「配置不可(-)判定」や「必要ランク制限」をバイパスし、未習得者でも配置可能とする。

### 5.9 館山病院 専従・代役配置ルール

館山病院への外部出勤に関する特殊な割り当てロジック。

| 項目 | 内容 |
|:---|:---|
| **専任担当者** | 遠藤 健太郎 (T066) |
| **バックアップ要員** | 福田 学 (T007)、石井 哲也 (T024)、箕輪 綱平 (T023) |
| **基本ロジック** | 遠藤が「休み希望（★や☆）」を出していない日は、必ず遠藤を「館山」に配置する。 |
| **代役ロジック** | 遠藤が休みの場合、バックアップ3名の中から「館山」の累積回数が少ない1名を自動選出する。 |
| **制約の優先順位** | 本人の休暇希望や、より優先度の高い特殊業務（HB等のAランク必須枠）が不足した場合は、館山への配置よりも欠員補充が優先される。 |
...
| 2.4 | 2026-04-04 | 業務拡大（育成ペアリング）機能の実装、勤務回数カウントの正規化（ク遅→ク等）、均等化ペナルティ重みの強化、月別公休数の動的設定機能を反映 |
