...
| 対象者 | 勤務場所 | ルール詳細 |
|--------|----------|------------|
| 動的判定 | 病MR, クMR, M遅 | ・マスター読み込み時に**「MRI系業務のスキルのみを有し、他のモダリティスキルを持たないスタッフ」**を動的に特定する。<br>・対象スタッフがMRI関連枠（病MR, クMR, M遅）に配置される際、通常の50倍にあたる超特大ボーナス（+500,000）を付与し、複数スキル担当者よりも優先してMRI枠を確保させる。 |
...
| 曜日 | クMR枠数 | ランク要件（ハード制約） | 育成・その他の要件（ソフト制約など） |
|------|------|------------|------|
